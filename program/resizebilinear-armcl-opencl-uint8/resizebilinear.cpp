...
int main() {
  ...
  measure_setup([&]() {
    TensorShape tensor_shape(static_cast<uint>(in_shape.width), static_cast<uint>(in_shape.height),
                             static_cast<uint>(in_shape.num));

    TensorShape shape_scaled(static_cast<uint>(out_shape.width), static_cast<uint>(out_shape.height),
                             static_cast<uint>(out_shape.num));

    input.allocator()->init(TensorInfo(tensor_shape, Format::U8));
    output.allocator()->init(TensorInfo(shape_scaled, Format::U8));
